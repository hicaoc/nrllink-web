<template>
  ...
  >
    ...
    <template v-else-if="kind === 'room'">
      ...
      <div class="hud-row">
        设备 {{ data.online_dev_number ?? 0 }} / {{ data.total_dev_number ?? 0 }}
      </div>
    </template>
...
export default {
  ...
  computed: {
    typeLabel() {
      ...
      return (
        lookupLabel(groupTypeOptions, this.data && this.data.type) ||
        `类型 ${this.data && this.data.type}`
      )
    },
    modelLabel() {
      return (
        lookupLabel(DevModelOptions, this.data && this.data.dev_model) ||
        `型号 ${this.data && this.data.dev_model}`
      )
    },
  },
}
